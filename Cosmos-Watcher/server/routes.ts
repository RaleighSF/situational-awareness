...
/**
 * Format scene context for the /reason API to properly distinguish between:
 * 1. Scene description (what to look for in the video)
 * 2. Available tools/capabilities (what can be recommended, NOT visible in video)
 * 
 * Convention: Use "TOOLS:" prefix in scene context to list available tools.
 * Example: "Industrial plant floor.\nTOOLS: Robotic dog with fire suppression capabilities"
 */
function formatContextForReason(sceneContext: string): string {
  if (!sceneContext || !sceneContext.trim()) {
    return "";
  }
  
  // Check for TOOLS: section (case-insensitive, using [\s\S] for ES2017 compat)
  const toolsMatch = sceneContext.match(/\bTOOLS?:\s*([\s\S]+?)(?=\n[A-Z]+:|$)/i);
  
  if (toolsMatch) {
    const toolsSection = toolsMatch[1].trim();
    // Remove the TOOLS section from the main context
    const mainContext = sceneContext.replace(/\bTOOLS?:\s*[\s\S]+?(?=\n[A-Z]+:|$)/i, '').trim();
    
    return `SCENE CONTEXT:
${mainContext}

AVAILABLE TOOLS/CAPABILITIES (not visible in video - recommend when appropriate):
${toolsSection}

NOTE: The tools listed above are available resources you may recommend deploying based on observed conditions. They are NOT present in the video frames - do not report on their status or look for them. When conditions warrant (e.g., fire detected), recommend their use in your escalation.`;
  }
  
  // Standard context without TOOLS section
  return sceneContext;
}

async function analyzeWithGemini(frameData: string, userPrompt: string): Promise<string> {
  if (!GEMINI_API_KEY) {
  ...
  const mimeType = dataUrlMatch ? `image/${dataUrlMatch[1]}` : "image/jpeg";
  const base64Data = dataUrlMatch ? dataUrlMatch[2] : frameData;
    ...
    contents: [
      {
        parts: [
          {
            text: `You are a precision visual analysis assistant. Your specialty is accurate counting and measurement of objects in images.
...
USER QUESTION: ${userPrompt}`
  ...
  console.log("[Gemini] --> Sending precision analysis request");
...
async function synthesizeObservations(observations: FrameObservation[], sceneContext?: string): Promise<{ synthesis: SceneAgentSynthesis | null; rawText: string }> {
  ...
  if (sceneContext) {
    const formattedContext = formatContextForReason(sceneContext);
    payload.context = formattedContext;
    console.log(`[REASON] Including formatted context: "${formattedContext.substring(0, 100)}..."`);
  }
...
async function synthesizeForAlert(
  ...
  // Format scene context to properly distinguish tools/capabilities from scene description
  let combinedContext = rulePrompt;
  if (sceneContext) {
    const formattedSceneContext = formatContextForReason(sceneContext);
    combinedContext = `${rulePrompt}\n\n${formattedSceneContext}`;
  }
...
export async function registerRoutes(
  ...
  // ---------- Frame extraction for VSS caption indexer ----------
  app.post("/api/capture-frame", async (req, res) => {
    try {
      const { url, time_offset } = req.body;
      if (!url || time_offset == null) {
        return res.status(400).json({ error: "Missing url or time_offset" });
      }

      // Resolve the file path — only allow local video files
      let filePath: string;
      if (url.startsWith("/attached_assets/")) {
        ...
        return res.status(400).json({ error: "Only local video files are supported for frame extraction" });
      }
      ...
      const frame_b64 = (stdout as Buffer).toString("base64");
      res.json({ frame_b64 });
    } catch (error: any) {
      // ffmpeg may fail if time_offset is past the end of the video — not an error worth logging loudly
      if (error?.stderr?.toString().includes("Output file is empty")) {
        return res.status(404).json({ error: "Frame at time offset not found (past end of video)" });
      }
      console.error("Error capturing frame:", error?.message || error);
  ...
  app.get("/objects/*", async (req, res) => {
    try {
      ...
      // First try to find in public paths
      const publicFile = await objectStorageService.searchPublicObject(objectPath);
      if (publicFile) {
        res.setHeader("Accept-Ranges", "bytes");
        return await objectStorageService.downloadObject(publicFile, res, 86400);
      }
