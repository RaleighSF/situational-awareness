...
export default function Dashboard() {
  ...
  // VSS New Features State
  const [isGridView, setIsGridView] = useState(false);
  const [isQueryPanelOpen, setIsQueryPanelOpen] = useState(false);
  const [explainAlertData, setExplainAlertData] = useState<{
    frameData: string;
    ruleText: string;
    confidence: string;
    cameraId?: string;
  ...
  // VSS Feature Handlers
  const handleExplainAlert = (alert: Alert) => {
    const prompt = prompts.find((p) => p.id === alert.promptId);
    setExplainAlertData({
      frameData: alert.frameData || "",
      ruleText: prompt?.prompt || "Unknown rule",
      confidence: alert.confidence || "MEDIUM",
      cameraId: currentVideoSourceId || undefined,
      sceneContext: sceneAgentContext || undefined,
      boundingBox: prompt?.boundingBox || null,
    });
    setIsExplainModalOpen(true);
  };

  const handleQueryResultClick = (cameraId: string, videoTimeSeconds: number) => {
    // Find the source matching the camera ID
    const source = videoSources.find(s => s.id === cameraId);
    if (source) {
      setCurrentVideoSourceId(source.id);
      setIsGridView(false);
  ...
  const handleTimelineSeek = (videoTimeSeconds: number) => {
    const video = document.querySelector('video');
    if (video) {
      video.currentTime = videoTimeSeconds;
    }
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="flex items-center justify-between gap-4 px-6 py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              className={isGridView ? "" : "text-primary"}
              data-testid="button-toggle-grid"
            >
              {isGridView ? (
                <><Monitor className="h-4 w-4 mr-1" /> Single</>
              ) : (
                <><LayoutGrid className="h-4 w-4 mr-1" /> Grid</>
              )}
            </Button>
            <Button
              variant={isQueryPanelOpen ? "default" : "outline"}
              size="sm"
              onClick={() => setIsQueryPanelOpen(!isQueryPanelOpen)}
              className={isQueryPanelOpen ? "" : "text-primary"}
      ...
      <main className="container max-w-7xl mx-auto p-6">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-medium">Multi-Camera View</h2>
                <Badge variant="secondary" className="text-xs">{videoSources.length} cameras</Badge>
              </div>
            </div>
            <CameraGrid
              videoSources={videoSources}
              onSelectSource={(sourceId) => {
                setCurrentVideoSourceId(sourceId);
                setIsGridView(false);
              }}
              currentSourceId={currentVideoSourceId}
        ...
        {/* NL Query Search Panel — slide-over from right */}
        <QueryPanel
      ...
      <AlertExplainModal
        ...
        onClose={() => {
          ...
          setExplainAlertData(null);
        }}
        alertData={explainAlertData}
      />
